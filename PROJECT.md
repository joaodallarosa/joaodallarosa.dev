...
_Last updated: 2026-07-07 · This file is the running source of truth for context, decisions, and "why." Hand this to a fresh Claude session before starting new work so it doesn't need to be re-explained._
...
**Still open (not blocking any phase so far):**
- i18n routing — subpath (`/fr/`, `/pt-br/`) vs. separate domains? Subpath is the default assumption; revisit when Phase 1's i18n scaffolding starts.
...
**Phase 1 — Core site shell + content pipeline**
Base Nuxt app, layouts per kind, Nuxt Studio installed/configured (drafts, Git integration), i18n scaffolding for en/fr/pt-BR (structure only, no translations yet).

**Phase 2 — Design system v0**
Small set of leaf components (Button, Tag, Card, Badge) in Lit, consumed via custom elements in Vue. Component showcase page live, consuming the real components. **React+Vue interop demo included here.**

**Phase 3 — Developer Mode**
`data-dev-mode` toggle; structural/x-ray skin built into the Phase 2 components; applied across site chrome.

**Phase 4 — Creative coding / WebGL layer**
Restrained, progressive-enhancement interactions with explicit performance budgets defined up front.

**Phase 5 — AI translation workflow v1**
